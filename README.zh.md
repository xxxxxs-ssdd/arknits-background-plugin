...
无。该插件只是浏览器端的一层绘制，外加一条静态图片路由，不会触达任何模型请求。
...
- **固定图片 URL**——绘制路径是包内字面量（`/arknights-wallpaper.webp`）；Node 半边从随包的 `arknights-wallpaper.webp` 提供该路径，换图只需替换包内这一个文件（无需改样式表或前端产物）。
